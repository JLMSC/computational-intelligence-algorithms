...
class ADALINE:
    ...
    def EQM(self, Xtrain: np.ndarray, Ytrain: np.ndarray) -> float:
        """Função para cálculo de Erro Quadrático Médio (EQM)

        Parameters
        ----------
        Xtrain : np.ndarray
            Variáveis independentes de conjunto treinamento.
        Ytrain : np.ndarray
            Variáveis dependentes de conjunto treinamento.
    ...
    def fit(self, epochs: int, lr: float, pr: float) -> None:
        ...
        self.W = np.zeros(self.p)
        current_epoch = 0
        EQM_prev, EQM_next = 0, 0
        while abs(EQM_prev - EQM_next) > pr or current_epoch < epochs:
            ...
            EQM_prev = self.EQM(Xtrain=Xtrain, Ytrain=Ytrain)
            for t in range(Xtrain.shape[1]):
                x_t = Xtrain[:, t]
                u_t = (self.W.T @ x_t)
                d_t = Ytrain[t, 0]
            ...
            current_epoch += 1
            EQM_next = self.EQM(Xtrain=Xtrain, Ytrain=Ytrain)
